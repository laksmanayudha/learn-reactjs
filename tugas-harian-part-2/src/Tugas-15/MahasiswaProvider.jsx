...
export const MahasiswaProvider = (props) => {
    ...
    const [alertStatus, setAlertStatus] = useState({alertPopUp: false, alertMessage: "", alertColor: ""});
    ...
    const popUpAlert = (status) => {
        if(status === "create"){
            setAlertStatus({alertPopUp: true, alertMessage: "Berhasil Menambah Data Mahasiswa", alertColor: "green"})
        }else if(status === "edit"){
            setAlertStatus({alertPopUp: true, alertMessage: "Berhasil Update Data Mahasiswa", alertColor: "green"})
        }else if(status === "delete"){
            setAlertStatus({alertPopUp: true, alertMessage: "Berhasil Delete Data Mahasiswa", alertColor: "red"})
        }else{
            setAlertStatus({alertPopUp: false, alertMessage: "", alertColor: ""})
        }
        setTimeout(() => {
            setAlertStatus({alertPopUp: false, alertMessage: "", alertColor: ""})
        }, 3000)
    ...
    const handleEdit = async (event) => {
        let mhsId = event.target.value;
        history.push(`/tugas15/edit/${mhsId}`)
    }
